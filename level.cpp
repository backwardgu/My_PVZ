...
bool level::plant_a_plant(plants &selected_plants,int &x, int &y)
{
	...
	lawn[line][row] = selected_plants.clone();		//通过智能指针拷贝获取植物
...
void level::mouse_click(ExMessage msg)
{
	...
	{
		if ( x > 338 && x < 338 + 65 * card_bar->chosen_plants.size() && y < 100)	//点击卡槽内
		{
		...
		else if (plants_picked)		//正拖拽着植物，检测是否能够种植
		{
	...
	if (msg.message == WM_MOUSEMOVE)		//植物图像跟随鼠标移动
	{
		if (plants_picked)
		{
...
void level::generate_zombie()
{
	if (waves.should_generate(pressure))
	{
		next_wave = waves.update();		//获取下一波次出怪列表
		for (int it : next_wave)
		{
			...
			case 0:id = zombie01_pool->get();
				...
				(*zombie01_pool)[id].use(900, LINE_START + 40 + LINE_WIDTH * line, line, WALK);
				new_zombie = &(*zombie01_pool)[id];
				break;
			}
			active_zombies[line].push_back(new_zombie);		//随机分配僵尸行数并加入激活对象池
		}
	}
}
...
void level::zombie_attack()
{
	for (int i = 0; i < 5 ; i++)
	{
		...
		{
			int line = i, row = 1 + (zombie->x - ROW_START) / ROW_WIDTH;
...
Zombies* level::find_target(int line, int x)		//如果索敌成功，则将目标指针返回给植物
{
	for (auto& its : active_zombies[line])
	{
			if (its->x <= x - 20) { continue; }
			return its;
	}
	return NULL;
}
	...
	for (int i = 0;i<5;i++)
	{
		for (auto bullets : active_bullets[i])
		{
			...
			for (auto zombie : active_zombies[i])
			{
				...
				{
					std::cout << "子弹行：" << i << "  僵尸行" << zombie->line;
					bullets->hit = true;
